...
```{r}
arrange(babynames, desc(prop), desc(n))
```
...
```{r}
babynames %>% summarise(minyear = min(year), lastyear = max(year), total = sum(n))
```
...
babynames %>%
    ...
    summarize(total=sum(n), firstyear=min(year))
```
...
babynames %>%
    ...
    summarize(total=sum(n)) %>%
    ungroup() %>%
    ggplot() + geom_line(mapping = aes(year, total))
```
...
```{r}
babynames2 <- (babynames %>% mutate (percent = round(prop*100),2))
```
